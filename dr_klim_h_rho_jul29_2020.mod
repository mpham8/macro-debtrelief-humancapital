/* dr_klim_h_rho_jul29_2020.mod [Dynare model file]
 * Based on debtrelief_klim_h_jul29_2020.mod
 * Incorporates rho as an exogenous variable, so policy experiment can
 * either be a cut in tau or a cut in rho.
 * By Steve O'Connell, July 29, 2020, updated Aug 2, 2020.
 *
...
var y d c r ikg sk k ihg sh h v dgap z dl htok;

// Exogenous variable: TFP, tax rate on GDP, debt cancellation
varexo x tau rho eps_dc;

// Parameters declaration and calibration
parameters alphak alphah gam delt mu g rw taubase dk;
...
tau = 0.20;
k = ( (x*(1-tau)*alphak^(1-alphah)*alphah^alphah)/((delt+rho-dk*(rho-rw))^(1-alphah)*(delt+rho)^alphah) )^(1/(1-alphak-alphah));
h = (alphah/alphak)*((delt+rho-dk*(rho-rw))/(delt+rho))*k;
y = x*k^alphak*h^alphah;
ikg = delt*k;
ihg = delt*h;
