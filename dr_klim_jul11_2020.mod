/* dr_klim_jul11_2020.mod [Dynare model file]
 * Based on debtrelief_klim_jul11_2020.mod
 * This version uses the analytical expression for eta that delivers
 * dss3 = 0.4*yss3.
 * 
 * debtrelief_klim_jul8_2020.mod [Dynare model file]
 * Run setpathdynare457_soc.m first.
 * By Steve O'Connell / July 8, 2020, updated August 2, 2020.
 *
 * This replaces the Detragiache (1992) specification d<=ds*y with the 
 * Cohen and Sachs 1986 specification d<=ks*k. The endogenous debt limit 
 * becomes dl = dk*k (exogenous variable dk is determined by parameter
 * ds by putting a steady-state version of the debt limit into the 
 * structural model).
 * 
 * In the previous version (debtrelief_jul8_2020) I just used dk = 0.3821 
 * which is the implied value in the related case of d<=ds*y.
*/
...
parameters alph gam delt rho mu g rw taubase dk;
...
taubase=0.20; //Needed to calculate dk to yield D=0.4 of GDP in baseline SS
dk = 0.4*(rho+delt)/((1-taubase)*alph+0.4*(rho-rw));  // yields 0.3750 ==> 0.4 of GDP in SS
//dk=0.4;  //debt limit: a guess, trying to be consistent with d = 0.4*y
...
s = dk*v + (1+rho)^(-1)*( c(+1)^(-gam)*(1-tau)*alph*x(+1)*k^(alph-1)
...
eps_dc = 0;
tau = 0.20;
k = (x*alph*(1-tau)/(delt+rho-dk*(rho-rw)))^(1/(1-alph)); //Modified
ig = delt*k;
y = x*k^alph;
c = y - ig - g - rw*d;
s = c^(-gam);
